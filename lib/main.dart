...
class _TrmStaListNowState extends State<TrmStaListNow> {
  ...
  void _download() {
    setState(() {
        Future<Downloaded> ourdownloaded() async {
          return await download((String url) => http.read(url));
        }
        ourdownloaded().then((Downloaded down) {
          _when = down.time;
  ...
  Widget build(BuildContext context) {
    return new Scaffold(
      ...
      body: new ScrollableList(
        children: _lista==null?[]:_lista.map((AllSta our) => new Text(our.toString(), key: new ValueKey<int>(our.locrow.stanum))),
        itemExtent: 50.0
      ),
